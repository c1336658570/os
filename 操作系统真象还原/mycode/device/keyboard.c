...
//用转义字符定义部分控制字符
//esc和delete没有一般转义字符的形式，因此只能考虑八进制或十六进制形式
...
//以下不可见字符一律定义为0，这类键是没有ASCII的，在控制键中只有字符控制键才有ASCII码。
//后面定义的数组中要用到ASCII码，为了与数组保持格式上的一致，所以将其统一定义为0，相当于占位用的
...
//keymap中每一个一维数组第0个元素是某个按键未与shift键组合时对应的字符ASCII码值，
//第1个元素是某个按键与shift键组合时对应的字符ASCII码值。
...
static void intr_keyboard_handler(void) {
  ...
  if (scancode == 0xe0) {
    ext_scancode = true;  //打开e0标记  用变量ext_scancode作为0xe0扩展扫描码的标记
    return;
  }
  ...
  //第一套键盘扫描码通码和断码的区别就是扫描码第8位的值，断码的第8位为1，通码的第8位为0，据此来获得是通码还是断码
  ...
  if (break_code) {   //若是断码break_code（按键弹起时产生的扫描码）
    ...
    //所以可用下面的方法取make_code（通码），多字节的扫描码暂不处理
  ...
  //若为通码，只处理数组中定义的键以及alt_right和ctrl_right键，全是make_code
  //最大支持的通码为0x3a，最小是0x01所以通过此进行判断，
  //而且还支持alt_r_make和ctrl_r_make，它们的通码以0x0e开头，共2字节，在0x3a的范围外，所以需要单独处理
  else if ((scancode > 0x00 && scancode < 0x3b) || \
    (scancode == alt_r_make) || \
    (scancode == ctrl_r_make)) { 
    bool shift = false;   //记录shift的状态，值为1表示按下，值为0表示弹起
    ...
    //通过else处理字母键 
      ...
      if (shift_down_last && caps_lock_last) {  //同时按下<shift>和<caps lock>，抵销了大写功能
        shift = false;
      } else if (shift_down_last || caps_lock_last) {
        shift = true;   //如果<shift>和<caps lock>仅按下了一个，大写功能开启
      } else {
        shift = false;
      }
    }
    //将扫描码的高字节置0，主要针对高字节是e0的扫描码
    uint8_t index = (scancode &= 0x00ff); //用0x00ff是因为此扫描码可能是2字节，高字节是0xe0，我们要抹去它
