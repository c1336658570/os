...
//接受两个参数，转换前的旧绝对路径old_abs_path和转换后的新绝对路径
//new_abs_path，功能是将路径old_abs_path中的“..”和“.”转换为实际路径后存入new_abs_path
//其中old_abs_path肯定是绝对目录，这是由主调函数传入的，因此new_abs_path必然也是绝对路径
static void wash_path(char *old_abs_path, char *new_abs_path) {
  assert(old_abs_path[0] == '/');
  char name[MAX_FILE_NAME_LEN] = {0};   //用它来存储路径中解析出来的各层目录名
  char *sub_path = old_abs_path;        //用它来配合函数path_parse的路径解析
  sub_path = path_parse(sub_path, name);
  //如果name为空，一定是old_abs_path仅为1个以上的“/”，此时把它当根目录处理，将 new_abs_path 填充为根目录“/”后返回
  ...
  strcat(new_abs_path, "/");  //接一个“/”，作为路径分隔符
  while (name[0]) {
    //如果是上一级目录“..”
    if (!strcmp("..", name)) {
...
//接受2个参数，原目录path和转换后的绝对路径final_path。其中path是用户键入的路径，可能是相对路径，
//也可能是绝对路径，也可能包含“.”和“..”的相对路径或绝对路径，而final_path是不含“.”和“..”的绝对路径。
void make_clear_abs_path(char *path, char *final_path) {
  char abs_path[MAX_PATH_LEN] = {0};
  //先判断是否输入的是绝对路径
  if (path[0] != '/') {   //若输入的不是绝对路径，就拼接成绝对路径
    memset(abs_path, 0, MAX_PATH_LEN);
    if (getcwd(abs_path, MAX_PATH_LEN) != NULL) {   //将当前工作路径存入abs_path
