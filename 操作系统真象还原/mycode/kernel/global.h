...
//代码段和数据段属于存储段，tss和各种门描述符属于系统段为1时表示存储段，为0时表示系统段
...
//定义GDT中描述符的结构
struct gdt_desc {
   uint16_t limit_low_word;         //GDT表界限的低16位
   uint16_t base_low_word;          //GDT表基址的低16位
   uint8_t  base_mid_byte;          //GDT表基址的中间8位
   uint8_t  attr_low_byte;          //GDT属性的低8位
   uint8_t  limit_high_attr_high;   //GDT表界限的高4位和属性的高4位
   uint8_t  base_high_byte;         //GDT表基址的高8位
};
...
#define IDT_DESC_16_TYPE 0x6  //16位的门，未使用，定义它只为和32位门区分
