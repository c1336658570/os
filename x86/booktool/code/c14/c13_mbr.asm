...
;全局描述符表（GDT）是不可或缺的，和从前一样，我们将它定义在从物理地址0x00007E00开始的地方，紧挨着初始化代码段。GDT 可大可小，最大能达到64KB，所以，它的空间一定要留够。内核程序的大小也是不定的，但可以规定它的起始位置。在这里，我们决定将它加载到从物理内存地址0x00040000 开始的地方。从这个地方往上，一直到0x0009FFFF，都是它的地盘，取决于它到底有多大，想用多少就用多少。从0x000A0000 往上，是ROM BIOS，硬件专有的。从0x000B8000 往上的32KB，是文本模式的显示缓冲区。
 ...
 setup:   ;保护模式下的代码段只是用来执行的，是否能读出，取决于其描述符的类别字段，但无论如何它都不能写入。
         ...
         mov ecx,0x00409800                 ;字节粒度的代码段描述符 DPL=0  该段的一个属性
         ...
         mov ecx,0x00409200                 ;字节粒度的数据段描述符 DPL=0  该段的一个属性
         call make_gdt_descriptor  ;创建段描述符
