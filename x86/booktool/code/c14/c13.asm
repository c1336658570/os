...
start:   ;进入3特权级的用户程序局部空间时，任务的I/O特权级IOPL是0，任务没有I/O操作的特权。
...
;上面那条条指令行不通。当控制转移到内核时，当前特权级没有变化，还是3，因为使用jmp far指令通过调用门转移控制是不会改变当前特权级别的。以这样低的特权级别来内核执行第867、868 行的指令，一定会引发处理器异常中断     
code_end:
