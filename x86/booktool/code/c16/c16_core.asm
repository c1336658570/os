...
allocate_a_4k_page:                         ;分配一个4KB的页
         ...
         push edx
         push ds
;本章中，没有检测实际可用内存的代码，仅仅假定我们只有2MB的物理内存可用。2MB的内存，可分为512个页，需要512个比特的位串。
         mov eax,core_data_seg_sel ;332、333 行，先令段寄存器DS 指向内核数据段。
         mov ds,eax
...
;如果对应的页目录项不存在，那么，将执行第379～381行的指令，以分配一个物理页作为页表，并将页的物理地址填写到页目录项内。
         ...
         or eax,0x00000007 ;US＝1，特权级别为3 的程序也可以访问；RW＝1，页是可读可写的；P＝1，页已经位于内存中，可以使用。内核的页表为什么允许特权级别为3 的程序访问呢？当然了，原则上是不允许的，但是，这个例程既要用于为内核分配页面，也要用于为用户任务分配页面。
...
create_copy_cur_pdir:                       ;创建新页目录，并复制当前页目录内容
         ...
         push ebx
         push ecx
;令段寄存器DS和ES都指向0～4GB数据段。
         mov ebx,mem_0_4_gb_seg_sel
         mov ds,ebx
         mov es,ebx
;420行，首先调用过程allocate_a_4k_page分配一个页，页的物理地址由EAX寄存器返回。
         call allocate_a_4k_page            
         mov ebx,eax
         or ebx,0x00000007 ;422行，将页物理地址的低12位改成属性，属性值为0x007，即，US＝1，允许特权级别为3的用户程序访问该页；RW＝1，页是可读可写的；P＝1，页位于物理内存中。
         mov [0xfffffff8],ebx ;为了能够访问到该页，我们把它的物理地址登记到当前页目录表的倒数第2个目录项。
;要访问这个新的页目录表，必须知道它的线性地址。事实上，它的线性地址是0xFFFFE000。事情是这样的，让我们倒过来分析一下。首先，线性地址0xFFFFE000 的页目录索引值是0x3FF，指向当前页目录表的最后一项；页表索引值为0x3FE，是页表内倒数第2项，存放的是页地址。当前页目录的最后一个目录项，存放的是页目录表自己的物理地址，页表就是当前页目录表自己，而倒数第2个目录项，又是新页目录表的物理地址。这就证明了，0xFFFFE000的确是新页目录表的线性地址。
...
terminate_current_task:                     ;终止当前任务
         ...
         mov ds,eax
;先根据EFLAGS 寄存器的NT 位，判断用户任务（当前任务）当初是怎么发起的。如果是由call 指令发起的，那么，就用iretd 指令转换到前一个任务（内核）；如果是由jmp 指令发起的，则直接用jmp 指令转换回内核任务。
         pushfd
         pop edx
...
;------------------------------------------------------------------------------- 
         pgdt             dw  0             ;用于设置和修改GDT 
                          dd  0
;始化了64字节的数据。这64字节首尾相连。用来表示512（64*8）个页的分配情况（已分配是1,未分配是0） 前32字节其中混杂了两字节的0x55，在物理地址0x00030000～0x00040000 之间，是一段较为连续的空闲区，共64KB，可划分为16 个页，页的物理地址为0x00030000～0x00040000，就对应着这两字节。本来，这两字节都应当是0x00，以表明是可以分配的空闲页。不过，为了表明大的、连续的线性地址空间不必对应着连续的页，同样的做法也出现在后面的64 个页中。
...
;592～602行，用于将当前页目录表的前半部分清空。在每次创建一个新任务时，都应当清空内核页目录表的前512个目录项。当前页目录表的后半部分是由内核使用的，内核的虚拟地址空间被映射在每个任务的高地址端，即0x80000000之后。
  ...
  .b1:
         ...
         jl .b1
;605、606 行，使段寄存器DS 指向内核数据段，准备从硬盘上把用户程序的第一个扇区读入内核缓冲区。
         ;以下开始分配内存并加载用户程序
         mov eax,core_data_seg_sel
...
;608～610行，从栈中取得用户程序所在的逻辑扇区号，和内核缓冲区的首地
         ...
         ;以下判断整个程序有多大
         mov eax,[core_buf]                 ;程序尺寸  用户程序的第一个双字就是它的大小，这是约定好的
         mov ebx,eax        ;现在的内存分配是按页进行的，所以最好使程序的大小能被4096整除
         and ebx,0xfffff000                 ;使之4KB对齐 
         add ebx,0x1000                        
         test eax,0x00000fff                ;程序的大小正好是4KB的倍数吗? 
         cmovnz eax,ebx                     ;不是。使用凑整的结果
;620、621 行，将程序的大小右移12次，相当于除以4096，这得到的是它占用的页数。下面要用ECX寄存器的内容来控制循环次数。
         mov ecx,eax
...
;循环的目的是分配物理页，并以4KB为单位读取用户程序来填充页。这里不是一个循环，而是两个，而且是嵌套的，即外循环和内循环。外循环负责分配4KB页
  ...
  .b2:
         ...
         add dword [es:esi+0x06],0x1000   ;更新线性地址
...
;644～652行，在内核的地址空间内分配内存，创建用户任务的TSS。
       ...
       ;为了后面的代码访问TSS，TSS 的线性基地址要登记到任务控制块（TCB）中。
...
;第661～667行，创建用户任务的代码段描述符，并登记到LDT中。
         ;建立程序代码段描述符 从程序中可见，代码段描述符中的基地址是0x00000000，段界限值是0x000FFFFF，粒度为4KB，因此，实际使用的界限值是0xFFFFFFFF。用户任务的特权级别是3，因此，代码段描述符的特权级别也是3，段选择子的特权级别也设置成3。
...
;669、670 行，将代码段描述符的选择子登记到任务状态段（TSS）中。TSS 的线性地址是从任务控制块（TCB）中取得的。
         mov ebx,[es:esi+0x14]              ;从TCB中获取TSS的线性地址
         mov [es:ebx+76],cx                 ;填写TSS的CS域 
;第673 ～ 679 行， 创建用户任务的数据段描述符。基地址为0x00000000，段界限也是0x000FFFFF，粒度为4KB。描述符特权级和段选择子的特权级都是3。
         ;建立程序数据段描述符
...
;三个栈段的基地址也是0x00000000，也要创建为向上扩展的数据段，段界限为0x000FFFFF，粒度为4KB。当然，这三个栈段，其描述符的特权级别不同，段选择子也不一样。
         ;在用户任务的局部地址空间内创建0特权级堆栈  698～749 行就是用来创建0、1、2 特权级的栈。
         mov ebx,[es:esi+0x06]              ;从TCB中取得可用的线性地址
         add dword [es:esi+0x06],0x1000
         call sys_routine_seg_sel:alloc_inst_a_page
...
;第805～820 行，填写任务状态段（TSS）的其余部分。包括LDT 选择子域、I/O 位映射区的偏移地址、前一任务的TSS 链接域、TSS 的界限、EIP 和EFLAGS 寄存器域。EIP 域填写的是用户程序的入口点，从内核任务切换到用户任务时，是用TSS 中的内容恢复现场的，所以这关系到任务应该从哪里开始执行。EFLAGS 域的内容是当前内核任务EFLAGS 寄存器的副本。
         ...
         mov dx,[es:esi+0x12]               ;段长度（界限） TSS长度
...
;第823～828 行，创建TSS 描述符，并登记到GDT 中。处理器要求TSS 描述符必须登记在GDT 中。TSS 描述符的特权级DPL 必须是0，只有当前特权级别为0 的程序才能转换到该任务。
         ...
         movzx ebx,word [es:esi+0x12]       ;段长度（界限）  0拓展
         mov ecx,0x00408900                 ;TSS描述符，特权级0
         call sys_routine_seg_sel:make_seg_descriptor
         call sys_routine_seg_sel:set_up_gdt_descriptor
         mov [es:esi+0x18],cx               ;登记TSS选择子到TCB
;剩下的事情，就是创建属于用户任务自己的页目录表。创建用户任务时，使用内核的页目录表，然后，再复制它，作为用户任务的页目录表。页目录表的复制工作是调用过程create_copy_cur_pdir完成的。该过程位于第406行，属于公共例程段。
         ;创建用户任务的页目录
         ;注意！页的分配和使用是由页位图决定的，可以不占用线性地址空间 
         call sys_routine_seg_sel:create_copy_cur_pdir  ;406行
         mov ebx,[es:esi+0x14]              ;从TCB中获取TSS的线性地址  将新页目录表的物理地址填写到用户任务TSS的CR3寄存器域中。
...
;939 行，修改页目录内第1个目录项的内容，使其指向页表，页表的物理地址是0x00021000，该页位于内存中，可读可写，但不允许特权级别为3的程序和任务访问。
         ;在页目录内创建与线性地址0x00000000对应的目录项   内核占用着内存的低端1MB，线性地址范围是0x00000000～0x000FFFFF，共256个4KB页，占用了页目录表的第1个目录项，以及该目录项下属页表的前256个页表项。
         mov dword [es:ebx+0],0x00021003    ;写入目录项（页表的物理地址和属性）      
;第942～952 行，对低端1MB建立恒等映射，将内存低端1MB 所包含的那些页的物理地址按顺序一个一个地填写到页表中，当然，仅填写256个页表项。第1个页表项对应的是线性地址0x00000000～0x00000FFF，填写的内容是第1 个页的物理地址0x00000000；第2 个页表项对应的是线性地址0x00001000～0x00001FFF，填写的是第2 个页的物理地址0x00001000；...
  ...
  .b2:   ;在实际写入每个页表项之前，先将页的物理地址转存到EDX寄存器，并将属性值加到其低12位上。
         mov edx,eax
         or edx,0x00000003  ;P＝1，RW＝1；US＝0，特权级别为3的程序和任务不能访问这些页。将属性值加到其低12位上。
         mov [es:ebx+esi*4],edx             ;登记页的物理地址
         add eax,0x1000                     ;下一个相邻页的物理地址 
         inc esi
         cmp esi,256                        ;仅低端1MB内存对应的页才是有效的 
         jl .b2
;前256个表项填写之后，esi寄存器的当前值是256，它又被用于第955～958 行，接着处理其余的表项，使它们的内容为全零。
  .b3:                                      ;其余的页表项置为无效
         mov dword [es:ebx+esi*4],0x00000000  
         inc esi
         cmp esi,1024
...
;961、962 行，将页目录表的物理基地址传送到控制寄存器CR3，也就是页目录表基地址寄存器PDBR
         ;令CR3寄存器指向页目录，并正式开启页功能   CR3(PDBR)是32位寄存器，3、4位是PWT和PCD，12-31位是页目录物理地址的12-31
...
;969～973 行，在内核的页目录表中，创建一个和线性地址0x80000000 对应的目录项，并使它指向同一个页表。毕竟，我们只改变了线性地址空间范围，内核的数据和代码仍然在原来的页内，没有改变。
         ;在页目录内创建与线性地址0x80000000对应的目录项  ebx高20位全为1，表示访问页目录表的最后一项和页表的最后一项
...
;973行，ES是指向0～4GB内存段的，EBX寄存器的内容为0xFFFFF000，ESI寄存器的内容为0x00000200，因此，段部件发出的线性地址是0xFFFFF200。当前程序或者任务的页目录表，其物理基地址是由控制寄存器CR3 指示的，仅高20 位有效，是多少并不重要，可以假定为0x?????000。段部件产生的线性地址是0xFFFFF200，其高10 位的值是0x3FF，这个值乘以4，结果为0xFFC。这个值同CR3 寄存器提供的页目录表物理地址相加，结果是0x?????FFC，它就是页目录表内最后一个目录项的物理地址。从此处取出一个双字，就是线性地址0xFFFFF200 所对应页表的物理地址。在前面第936 行，我们已经在该目录项内填写了页目录表的物理基地址。因此，该目录项所指向的页表正是当前的页目录表自己，这实际上是把页目录表当成页表来用。
;处理器用线性地址0xFFFFF200 的中间10 位作为偏移量访问页表，这10 位的值是0x3FF。页表的物理地址就是页目录表的物理地址，即0x?????000；表内偏移量是0x3FF 乘以4，即0xFFC。故，这一次处理器发出的最终物理地址也同样是0x?????FFC，它就是页表内最后一个页表项的物理地址。从此处取出一个双字，就是线性地址0xFFFFF200 所在的那个页的物理地址。因为访问的又是同一内存位置，故最终要访问的页仍是页目录表自己。页的物理地址是0x?????000，页内偏移由线性地址的低12 位乘以4 给出，其值为0x800。所以，当973行指令执行时，处理器发出的物理地址是0x?????800，并将该双字单元的内容改写为0x00021003。综合上面的分析，这就是说，如果页目录表的最后一个目录项指向当前页目录表自己，那么，无论任何时候，当线性地址的高20 位是0xFFFFF 时，访问的就是页目录表自己。
         ;将GDT中的段描述符映射到线性地址0x80000000，将所有描述符高字部分的最高位置“1”即可。
         sgdt [pgdt]
;977～979行，先取得GDT的线性基地址，并传送到EBX寄存器，准备开始访问GDT内的段描述符。
         mov ebx,[pgdt+2]
;981～986行，依次找到内核栈段、文本模式下的视频缓冲区段、公共例程段、内核数据段和内核代码段的描述符，并将每个描述符的最高位改成“1”。在这里，EBX寄存器提供了GDT的基地址
...
;创建用户任务的任务控制块  1055～1057行，用于在内核的虚拟地址空间里分配4KB的内存（页）
         ...
         call sys_routine_seg_sel:alloc_inst_a_page ;将EBX寄存器中的线性地址作为参数，调用过程alloc_inst_a_page去申请一个物理页。
         add dword [core_next_laddr],4096  ;将标号core_ next_laddr处的数据修改为下一个可分配的起始线性地址。
;1059～1062行，初始化TCB，为某些域赋初值  在TCB中，有两个项目应该在创建用户任务前就予以填写和初始化，它们是LDT当前界限值和下一个可用的线性地址。
         mov dword [es:ebx+0x06],0          ;用户任务局部空间的分配从0开始。 0x06-0x09存下一个可用的线性地址。（虚拟地址）每个任务都有自己的4GB 虚拟内存空间，线性地址范围是0x00000000～0xFFFFFFFF，它是任务自己的空间，可以任意分配和使用。
...
;1069～1072 行，先显示一条消息，告诉屏幕前的人，正在执行任务切换。
         mov ebx,message_4
         call sys_routine_seg_sel:put_string
;call指令的参数是TCB中的TSS选择子。
