...
;-------------------------------------------------------------------------------
;显示一个字符，先获取当前光标位置，然后判断字符是什么字符，如果是回车，就将光标移动到当前行开头，然后设置光标的位置，
;如果不是判断是否是换行，如果是换行，就将光标移动到下一行的这个位置，然后判断光标是否超出屏幕，超出就滚屏，
;将字符整体上移一行，将最后一行设为黑底白字空白字符，然后设置光标位置
;如果不是换行，就将该字符输出到屏幕，并修改光标位置，判断光标是否超出屏幕，如果超出就滚屏，
;将字符整体上移一行，将最后一行设为黑底白字空白字符，然后设置当前光标位置
put_char:                                ;显示一个字符
                                         ;输入：cl=字符ascii
         push ax
         push bx
