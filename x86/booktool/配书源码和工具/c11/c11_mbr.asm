         ...
         mov dword [bx+0x08],0x7c0001ff      ;线性地址0x00007c00，G=0粒度是字节，段界限0x001FF(512字节)，s=1属于存储器的段
         ...
         lgdt [cs: gdt_size+0x7c00]     ;操作数是16字节，低16位是GDT界限值(31)，高32位是GDT基地址(0x00007e00)。
      
         in al,0x92                     ;南桥芯片内的端口 ，读0x92端口，0x92位0写1导致处理器复位，位1用于控制A20
         or al,0000_0010B               ;将位1改为1，打开A20地址线     
         out 0x92,al                        ;打开A20,第21根地址线

         cli                                ;保护模式下中断机制尚未建立，实模式中断不再适用，应 
                                            ;禁止中断 
         ...
         ;以下进入保护模式... ...
         jmp dword 0x0008:flush             ;16位的描述符选择子：32位偏移（清空段寄存器的高速缓存器，因为它在实模式下也使用了，里面的内容在保护模式可能出错，需尽快清除）。
    ...
    flush:
         ...
         mov ss,cx
         mov esp,0x7c00
          ;当前程序的代码段，其描述符的D 位是“1”，所以，当进行隐式的栈操作时，默认地，每次压栈操作时，压入的是双字；当前程序所使用的栈段，其符述符的B 位也是“1”，默认地，使用栈指针寄存器ESP 进行操作。
         mov ebp,esp                        ;保存堆栈指针 
         push byte '.'                      ;压入立即数（字节），当前正在执行的代码段是32位，D位是1，栈段B位也是1，使用ESP，push指令默认操作数是32位，byte告诉编译器，该指令对应的格式为push imm8，必须使用操作码0x6A，而不是用来在编译后的机器指令前添加指令前缀。因此，该指令实际在处理器上执行时，压入栈中的是一个双字，也就是4 字节，高24位是该字节符号的扩展。
