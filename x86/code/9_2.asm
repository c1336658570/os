;使用新的方法产生中断信号，建议周期性中断。这涉及分频电路的分节点，比如可以选择250ms或者500ms，分别会在1s产生4次或2次中断
;===============================================================================
...
;-------------------------------------------------------------------------------
start:
      mov ax,[stack_segment]    ;初始化段寄存器
      mov ss,ax
      mov sp,ss_pointer         ;切换栈的两条命令之间禁止中断
      mov ax,[data_segment]
      mov ds,ax
      ...
      out 0x70,al
      mov al,0x42                        ;设置寄存器B，打开周期性中断，禁止更 
